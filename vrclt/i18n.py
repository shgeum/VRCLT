...
STRINGS = {
    ...
    "reset_config_body": {
        "ko": "설정을 기본값으로 되돌리고 런타임을 재시작할까요?\n\n보존: 출력 언어 목록, 자막 언어 목록\n초기화: API 키, 장치, 오디오, OSC, VR 위치, 핫키 등",
        "en": "Reset settings to defaults and restart the runtime?\n\nKept: output language list, subtitle language list\nReset: API key, devices, audio, OSC, VR positions, hotkeys, and other settings",
        "ja": "設定を既定値に戻してランタイムを再起動しますか?\n\n保持: 出力言語リスト、字幕言語リスト\nリセット: APIキー、デバイス、オーディオ、OSC、VR位置、ホットキーなど",
        "zh": "将设置重置为默认值并重启运行时吗？\n\n保留：输出语言列表、字幕语言列表\n重置：API 密钥、设备、音频、OSC、VR 位置、热键等",
    },
