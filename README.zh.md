...
- 降低 passthrough 延迟: 原始麦克风音频直接使用捕获到的 48 kHz 流，并减少播放缓冲。
- 提升翻译响应速度: 更频繁地向 Gemini 发送麦克风音频，并在检测到真实静音后更早发送回合结束提示。
- OSC 聊天框实时输出: 说话过程中发送 partial 翻译文本，并抑制相同内容的 final 重复发送。
- Discord 捕获更稳定: 从根进程捕获 Discord 多进程树，所选 PID 变化时会重新启动捕获。
- 降低字幕延迟: 缩短入站 VAD hangover，加快实时字幕 partial 刷新，并更早确认字幕行。
- 模式切换更稳定: 在翻译和 passthrough 之间切换时清空旧的翻译音频、监听音频和 passthrough 缓冲。
- 更新提醒: 发布版会检查 GitHub Releases；有新版本时显示仪表板横幅、托盘操作和通知。
- 配置重置流程: 应用更新后，会询问一次是否在保留 API 密钥、输出语言列表、字幕语言列表、UI 语言、窗口关闭行为和已选择音频设备的同时恢复默认设置。同样的重置也可在设置中执行。
