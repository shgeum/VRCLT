...
- GitHub Releases 更新提醒，以及保留 API 密钥和已保存语言列表的安全配置重置
...
- PC 全局热键设置
- 音频阈值和 VAD 设置
- 保留 API 密钥、输出语言列表和字幕语言列表的恢复默认设置按钮
...
- 想重置设置: 使用设置标签页中的 **恢复默认设置**。它会保留 API 密钥、输出语言列表和字幕语言列表，并将其他设置恢复默认。应用更新后，vrclt 也会就此重置询问一次。
