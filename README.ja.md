...
- passthrough の遅延を削減: 元のマイク音声はキャプチャした 48 kHz ストリームを直接使い、再生バッファリングを減らしました。
- 翻訳の応答性を改善: マイク音声を Gemini へより頻繁に送り、実際の無音を検出した後はより早くターン終了ヒントを送ります。
- OSC チャットボックスのライブ出力: 発話中の partial 翻訳テキストを送信し、同じ内容の final 重複送信は抑制します。
- Discord キャプチャを安定化: Discord の複数プロセスツリーをルートプロセスからキャプチャし、選択 PID が変わった場合はキャプチャを再起動します。
- 字幕遅延を削減: 受信側 VAD hangover を短くし、ライブ字幕 partial の更新と字幕確定を早めました。
- モード切り替えを安定化: 翻訳と passthrough の切り替え時に、古い翻訳音声、モニター音声、passthrough バッファをクリアします。
- 更新通知: リリースビルドは GitHub Releases を確認し、新しいバージョンがある場合はダッシュボードバナー、トレイ操作、通知を表示します。
- 設定リセット: アプリ更新後、API キー、出力言語リスト、字幕言語リスト、UI 言語、ウィンドウを閉じる動作、選択したオーディオデバイスを保持したまま既定値に戻すかを 1 回確認します。同じリセットは設定からも実行できます。
